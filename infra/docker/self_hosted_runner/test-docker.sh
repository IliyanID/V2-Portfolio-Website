...
container=v2-portfolio-website-runner
image=docker-repo.iliyandimitrov.com/v2-portfolio-website-runner:latest
...
docker run -d \
-e CI=true \
-v $(pnpm store path):/root/.local/share/pnpm/store/v3 \
-v ${root}:/V2-Portfolio-Website \
--name ${container} ${image} \
/bin/sh -c "while true; do sleep 1; done"

#Attaching to container
printf "${blue}Attaching to container...${nc}\n"
docker exec -it ${container} /bin/sh -c "cd /V2-Portfolio-Website && /bin/bash"